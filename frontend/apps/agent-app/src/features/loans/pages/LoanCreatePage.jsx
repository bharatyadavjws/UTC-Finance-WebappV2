...
import { retailerRepository } from '../../retailers/services/retailerRepository';
...
export function calculateEmiOptions(itemValue, downPayment, deviceType = 'ANDROID', isExistingUser = false) {
  ...
  return filteredTenures.map((t) => {
    ...
    return {
      ...
      interest,
      totalRepay,
      firstEmiDate: firstDate.toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }),
      lastRepaymentDate: lastDate.toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }),
      avFee: AV_FEE,
      appLockFee: LOCK_FEE,
...
export function computeApprovedCreditLimit({ itemValue, downPayment, income, cibil, existingEmi }) {
  ...
  if (dpPct < requiredMinPct) {
    return { ok: false, error: `Minimum downpayment for this item value is ${requiredMinPct}% of item value.` };
  }
  ...
  if (foirCurrent > maxFoir) {
    return { ok: false, error: `FOIR exceeds the maximum allowed ${maxFoir}% for this income band.` };
  }
...
/* ---------- Success Screen ---------- */
function SuccessScreen({ loanCode, retailer, form, selectedPlan, onClose }) {
    ...
    <div>
      <div style={{ background: '#e8f5e9', border: '1px solid #c8e6c9', borderRadius: '12px', padding: '20px', marginBottom: '16px', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: '14px', color: '#2e7d32', marginTop: '4px' }}>
          Agreement ID: <strong>{loanCode}</strong>
        </div>
      </div>
          ...
          <div style={pdfPage}>
            <div style={{ borderBottom: '1px solid #000', paddingBottom: '10px', marginBottom: '15px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                  <h1 style={{ margin: 0, fontSize: '18px', fontWeight: '900' }}>UTC FINANCE PVT. LTD.</h1>
                  <div style={{ fontSize: '8px', marginTop: '4px' }}>
                    <strong>Corporate Office:</strong> Plot 47, Sagar Enclave, Rajendra Park, Gurgaon, Haryana-122001
                    <br />
                    <strong>Registered Office:</strong> Plot 47, Sagar Enclave, Rajendra Park, Gurgaon, Haryana-122001
                    <br />
                    <strong>CIN:</strong> U65910HR2024PTC042961 | <strong>Email:</strong> hello@utcfinance.com
                  </div>
                </div>
            ...
            <div style={{ marginTop: '20px' }}>
              ...
              <p style={{ marginTop: '15px' }}>
                Dear <strong>{form.customerName}</strong>,
                <br /><br />
                We thank you for choosing <strong>UTC FINANCE PVT. LTD.</strong> as your financial partner.
              </p>
            ...
            <table style={kfsTable}>
              <thead>
                <tr>
                  ...
                  <th style={kfsTh}>Parameter</th>
                  <th style={kfsTh}>Details</th>
                </tr>
              </thead>
              <tbody>
                ...
                <tr><td style={kfsTd}>4</td><td style={kfsTd}>Loan Amount (To be disbursed to dealer)</td><td style={kfsTd}>{inr(disbursement)}</td></tr>
                <tr><td style={kfsTd}>5</td><td style={kfsTd}>Loan Term</td><td style={kfsTd}>{tenure} Months</td></tr>
                <tr><td style={kfsTd}>6</td><td style={kfsTd}>Monthly EMI Amount</td><td style={kfsTd}>{inr(emiAmount)}</td></tr>
                <tr><td style={kfsTd}>7</td><td style={kfsTd}>Processing Fees</td><td style={kfsTd}>{inr(selectedPlan?.processingFee)}</td></tr>
              </tbody>
            </table>
          </div>
...
export default function LoanCreatePage() {
  ...
  const retailerFromState = location.state?.retailer || null;
  const retailerId = searchParams.get('retailer_id');

  const [retailer, setRetailer] = useState(retailerFromState);
  ...
  useEffect(() => {
    if (retailerFromState) {
      setRetailer(retailerFromState);
      return;
    }
  ...
  const validateStep2 = () => {
    ...
    if (!/^[A-Z]{5}\d{4}[A-Z]{1}$/.test(form.panNumber)) return 'Enter a valid PAN.';
  ...
  useEffect(() => {
    ...
    const effectiveDownPayment = Math.max(itemValueNum - creditLimit, 0);

    const opts = calculateEmiOptions(itemValueNum, effectiveDownPayment, form.deviceType, false);
  ...
  const handleCreateLoan = async (e) => {
    ...
    if (!isConfirmed) {
      setError('You must accept the loan terms to create the loan.');
      return;
    }
    ...
    try {
      ...
      const response = await fetch('https://usethecred.com/api/loans', {
        method: 'POST',
        headers: {
        ...
        <div style={{ display: 'grid', gridTemplateColumns: gridColumns, gap: '24px', alignItems: 'start' }}>
          <div style={{ border: '1px solid #dbe2ea', borderRadius: '16px', padding: isMobile ? '16px' : '20px', background: '#ffffff' }}>
            ...
            <form onSubmit={handleCreateLoan} style={{ display: 'grid', gap: '16px' }}>
              {step === 1 && <StepCustomerDetails form={form} onTextChange={handleTextChange} onFileChange={handleFileChange} isMobile={isMobile} />}
              {step === 2 && <StepKycDetails form={form} onTextChange={handleTextChange} onFileChange={handleFileChange} isMobile={isMobile} />}
              {step === 3 && <StepItemDetails form={form} onTextChange={handleTextChange} onFileChange={handleFileChange} isMobile={isMobile} />}
              {step === 4 && (
                <StepDownpaymentEmi
                  form={form}
              ...
              <div style={{ display: 'grid', gap: '12px' }}>
                ...
                <div style={{ padding: '14px', borderRadius: '12px', border: '1px solid #e5e7eb', background: '#fafafa' }}>
                  ...
                  <div style={{ fontSize: '14px', color: '#4b5563' }}>
                    First EMI: <strong>{selectedPlan.firstEmiDate}</strong>, Last EMI: <strong>{selectedPlan.lastRepaymentDate}</strong>
                  </div>
                </div>
...
function StepCustomerDetails({ form, onTextChange, onFileChange, isMobile }) {
  return (
    <section>
      <h3 style={{ fontSize: isMobile ? '20px' : '22px', marginBottom: '10px', color: '#111827' }}>Customer Details</h3>
      <div style={{ display: 'grid', gap: '12px' }}>
...
function StepKycDetails({ form, onTextChange, onFileChange, isMobile }) {
  return (
    <section>
      <h3 style={{ fontSize: isMobile ? '20px' : '22px', marginBottom: '10px', color: '#111827' }}>KYC Details</h3>
      <div style={{ display: 'grid', gap: '12px' }}>
        <div style={fieldGroupStyle}>
...
function StepItemDetails({ form, onTextChange, onFileChange, isMobile }) {
  return (
    <section>
      <h3 style={{ fontSize: isMobile ? '20px' : '22px', marginBottom: '10px', color: '#111827' }}>Device Details</h3>
      <div style={{ display: 'grid', gap: '12px' }}>
        <div style={fieldGroupStyle}>
...
function StepDownpaymentEmi({
  ...
  return (
    <section>
      <h3 style={{ fontSize: isMobile ? '20px' : '22px', marginBottom: '10px', color: '#111827' }}>Downpayment & EMI</h3>
      <div style={{ display: 'grid', gap: '12px' }}>
        <div style={fieldGroupStyle}>
          <label style={labelStyle}>Item Value</label>
          <input type="number" name="itemValue" style={inputStyle} placeholder="Total device price" value={form.itemValue} onChange={onTextChange} />
        ...
        <div style={{ marginTop: '12px' }}>
          <div style={{ fontSize: '14px', fontWeight: '600', marginBottom: '6px', color: '#111827' }}>Available EMI Options</div>
  ...
  return (
    <section>
      ...
      <div style={blockStyle}>
        ...
        <div style={lineStyle}>Downpayment: {Number.isFinite(adjustedDownPayment) ? `₹ ${adjustedDownPayment.toLocaleString('en-IN', { maximumFractionDigits: 0 })}` : '-'}</div>
        <div style={lineStyle}>Net Disbursement: {Number.isFinite(disbursementAmount) ? `₹ ${disbursementAmount.toLocaleString('en-IN', { maximumFractionDigits: 0 })}` : '-'}</div>
      </div>
